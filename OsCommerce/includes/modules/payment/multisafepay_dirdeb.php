...
class multisafepay_dirdeb extends multisafepay {
    ...
    function update_status() {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_MSP_DIRDEB_ZONE > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MSP_DIRDEB_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
