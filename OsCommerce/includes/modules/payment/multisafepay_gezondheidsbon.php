...
class multisafepay_gezondheidsbon extends multisafepay
{
	...
	function multisafepay_gezondheidsbon()
	{
		...
		$this->title       		= 	$this->getTitle('GEZONDHEIDSBON');
		$this->public_title     = 	$this->getTitle('GEZONDHEIDSBON');
	...
	{
		if ($this->enabled && ((int)MODULE_PAYMENT_MSP_GEZONDHEIDSBON_ZONE > 0) ) 
		{
			$check_flag  	=	 false;
			$check_query 	= 	tep_db_query("SELECT zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '" . MODULE_PAYMENT_MSP_GEZONDHEIDSBON_ZONE . "' AND zone_country_id = '" . $GLOBALS['order']->billing['country']['id'] . "' ORDER BY zone_id");
	...
	function process_button()
	{
		...
		return tep_draw_hidden_field('msp_paymentmethod', 'GEZONDHEIDSBON');
	...
	function check()
	{
		...
		{
			$check_query 	= 	tep_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_MSP_GEZONDHEIDSBON_STATUS'");
	...
	function install()
	{
		...
		//tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Direct iDeal', 'MODULE_PAYMENT_MSP_GEZONDHEIDSBON_DIRECT', 'True', 'Select the bank within the website?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
	}
	...
	function keys()
	{
		...
		(
			...
			//'MODULE_PAYMENT_MSP_GEZONDHEIDSBON_DIRECT'
		);
	}
}
?>