...
class multisafepay_maestro extends multisafepay {
    ...
    function update_status() {
        if ($this->enabled && ((int) MODULE_PAYMENT_MSP_MAESTRO_ZONE > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("SELECT zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '" . MODULE_PAYMENT_MSP_MAESTRO_ZONE . "' AND zone_country_id = '" . $GLOBALS['order']->billing['country']['id'] . "' ORDER BY zone_id");
    ...
    function keys() {
        return array
            ...
            'MODULE_PAYMENT_MSP_MAESTRO_ZONE',
                //'MODULE_PAYMENT_MSP_MAESTRO_DIRECT'
        );
    }
