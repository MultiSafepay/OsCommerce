...
$check_query = tep_db_fetch_array(tep_db_query("select countries_iso_code_2 from " . TABLE_COUNTRIES . " where countries_id = '" . SHIPPING_ORIGIN_COUNTRY . "'"));
...
{
    echo 'There are multiple shipping methods actives with different tax classes. Please ensure all the methods use the same tax class';
    exit();
}

//Dfault tax (for shipping)

if (sizeof($tax_class_unique) == 1)
{
    $tax_rates_result = tep_db_query("select * from " . TABLE_TAX_RATES . " as tr where tr.tax_class_id= '" . $tax_class_unique[0] . "'");
...
header('Location: ' . $msp->_start_fastcheckout());
exit;
?>