...
class multisafepay_brouwmarkt extends multisafepay
{
    ...
    function __construct()
    {
        ...
        $this->title = $this->getTitle('Brouwmarkt');
        $this->public_title = $this->getTitle('Brouwmarkt');
        $this->description = $this->description = "<img src='images/icon_info.gif' border='0'>&nbsp;<b>MultiSafepay Brouwmarkt</b><BR>The main MultiSafepay module must be installed (does not have to be active) to use this payment method.<BR>";
        $this->enabled = MODULE_PAYMENT_MSP_BROUWMARKT_STATUS == 'True';
        $this->sort_order = MODULE_PAYMENT_MSP_BROUWMARKT_SORT_ORDER;
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_MSP_BROUWMARKT_ZONE > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MSP_BROUWMARKT_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function process_button()
    {
        ...
        return tep_draw_hidden_field('msp_paymentmethod', 'BROUWMARKT');
    }
